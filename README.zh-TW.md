...
| **圖像生成** | Stable Diffusion Web UI（獨立程式） | 透過 OpenRouter（chat completions + `modalities: ["image"]`）恢復支援 — 在 `config.toml` 的 `[image_generation]` 區段設定 |
...
[image_generation]
# OpenRouter 透過 chat completions 端點提供圖像生成，而非 /images/generations。
# 設定 enabled = false 可完全停用場景插圖功能。
enabled  = true
api_base = "https://openrouter.ai/api/v1"
model    = "bytedance-seed/seedream-4.5"
# 其他可用模型："google/gemini-2.0-flash-exp:free"、"recraft-ai/recraft-v3"
```
...
IMAGE_API_KEY=sk-...          # 省略時自動沿用 LLM_API_KEY
...
- **劇情摘要** — 每次代理回應後，Cocai 以簡短摘要更新左側歷史面板。LLM 會先判斷最近的交流是否推進了故事，因此純規則說明或閒聊不會觸發更新。可設定 `[auto_update] history = false` 停用。
- **場景插圖** — 當場景或場所發生明顯變化（移動至新地點、時間推移等），Cocai 會透過設定的圖像服務生成插圖並顯示於中央面板。可設定 `[auto_update] scene = false` 停用自動偵測，或設定 `[image_generation] enabled = false` 完全關閉圖像生成。
...
**場景插圖未顯示 / 圖像生成 404** — 大多數純 LLM 服務（包含 OpenRouter 的一般 LLM 端點）**不**提供 `/images/generations`。OpenRouter 的圖像模型需透過 chat completions 端點搭配 `modalities: ["image"]` 參數呼叫——Cocai 已自動處理此邏輯。若使用其他 `[image_generation] api_base`，請確認該服務確實支援圖像生成。如需完全停用插圖功能，設定 `[image_generation] enabled = false` 即可。
